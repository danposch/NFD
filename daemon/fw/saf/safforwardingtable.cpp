...
NFD_LOG_INIT("SAFForwardingTable");
...
int SAFForwardingTable::determineNextHop(const Interest& interest, std::vector<int> alreadyTriedFaces)
{
  ...
  for(std::vector<int>::iterator i = alreadyTriedFaces.begin (); i != alreadyTriedFaces.end ();++i)
  {
    ...
    if(offset != FACE_NOT_FOUND)
    {
      ...
      face_list.erase (face_list.begin ()+ (unsigned int) offset);
    }
...
void SAFForwardingTable::update(boost::shared_ptr<SAFStatisticMeasure> stats)
{
  ...
  {
    NFD_LOG_DEBUG("Updating Layer[" << layer << "] with reliability_t=" << curReliability[layer]);
    ...
    for(std::vector<int>::iterator it = r_faces.begin(); it != r_faces.end(); ++it)
      NFD_LOG_DEBUG("Reliable Face[" << *it << "]=" << stats->getFaceReliability(*it,layer)
                   << "\t "<< stats->getForwardedInterests (*it,layer) << " interest forwarded");
    for(std::vector<int>::iterator it = ur_faces.begin(); it != ur_faces.end(); ++it)
      NFD_LOG_DEBUG("Unreliable Face[" << *it << "]=" << stats->getFaceReliability(*it,layer)
                   << "\t "<< stats->getForwardedInterests (*it,layer) << " interest forwarded");
    for(std::vector<int>::iterator it = p_faces.begin(); it != p_faces.end(); ++it)
      NFD_LOG_DEBUG("Probe Face[" << *it << "]=" << stats->getFaceReliability(*it,layer)
                   << "\t "<< stats->getForwardedInterests (*it,layer) << " interest forwarded");
    NFD_LOG_DEBUG("Drop Face[" << DROP_FACE_ID << "]=" << stats->getFaceReliability(DROP_FACE_ID,layer)
    ...
    if(utf > 0)
    {
      if(r_faces.size () > 0)
      {
        ...
        {
          NFD_LOG_DEBUG("Face[" << *it <<"]: getS() / curReliability[*it] = "
          ...
          ts_sum += ts[*it];
          NFD_LOG_DEBUG("Face[" << *it <<"]: still can take " <<  ts[*it] << " more Interests");
        }
        NFD_LOG_DEBUG("Total Interests that can be taken by F_R = " << ts_sum);
        ...
        NFD_LOG_DEBUG("Total fraction that will be shifted to F_R= " << min_fraction);

        //now shift traffic to r_faces
        for(std::vector<int>::iterator it = r_faces.begin(); it != r_faces.end(); ++it) // for each r_face
        {
          NFD_LOG_DEBUG("Face[" << *it <<"]: Adding (min_fraction*ts[" << *it << "] / I) / (ts_sum / I)="
      ...
      NFD_LOG_DEBUG("UTF remaining for the Dropping Face = "<< utf);
...
void SAFForwardingTable::probeColumn(std::vector<int> faces, int layer, boost::shared_ptr<SAFStatisticMeasure> stats)
{
  ...
  NFD_LOG_DEBUG("Probing! Probe Size = p(F_D) * rho * lweight= " << table(determineRowOfFace (DROP_FACE_ID), layer)
                 << "*" << stats->getRho (layer) << " * "<< lweight << "=" << probe);

  if(probe < 0.001) // if probe is zero return
    return;

  /*NFD_LOG_DEBUG("Probing! Probe Size = p(F_D) * rho = " << table(determineRowOfFace (DROP_FACE_ID), layer)
...
void SAFForwardingTable::crossLayerAdaptation(boost::shared_ptr<SAFStatisticMeasure> smeasure)
{
  ...
  for(int layer = 0; layer < (int)ParameterConfiguration::getInstance ()->getParameter ("MAX_LAYERS") - 1; layer++) // -1 as last layer can not be adapted anyway
  {
    ...
    else if(table(determineRowOfFace(DROP_FACE_ID),layer) > 0)
    {
      ...
      NFD_LOG_DEBUG("Calculating number of periods to wait for layer " << layer << " to stabilize");
      ...
      for(std::vector<int>::iterator it = ur_faces.begin (); it != ur_faces.end (); ++it)
      {
        ...
        if(satisfied_interests < 1)
        {
          ...
          NFD_LOG_DEBUG( "n["<< *it << "] = skipped, \t We think it is a probing face");
          continue;
        }

        if( p0 * total_interests < satisfied_interests)//means the update procedure made the face already reliable
        {
          NFD_LOG_DEBUG( "n["<< *it << "] = skipped, \t We think the previous update procedure made the face already reliable");
          continue;
        }
        ...
        NFD_LOG_DEBUG( "n["<< *it << "] = [ ln(S(i)/t[i] - S(i)) - ln(p(i)*I - S(i))] / ln(1 - ema_alpha(i)) = "
      ...
      NFD_LOG_DEBUG("Layer " << layer << " is under observation for std::min(" << n_max << "," << MAX_OBSERVATION_PERIODS << ") steps") ;
  ...
  {
    NFD_LOG_DEBUG("Observation Phase for layer " << *it << " is over. Dropping traffic will be shifted\n");
    int curLayer = *it;
...
void SAFForwardingTable::updateReliabilityThreshold(int layer, bool increase)
{
  ...
  {
  if(increase)
    NFD_LOG_DEBUG("Increasing reliability[" << layer << "]=" << new_t);
  else
    NFD_LOG_DEBUG("Decreasing reliability[" << layer << "]=" << new_t);
  }

  curReliability[layer] = new_t;
}

double SAFForwardingTable::nextRandom ()
{
